...
def file_encrypt(data,secret):
    ...
    mac = hmac.new(kmac, ciphertext, hashlib.sha256).digest()

    tag = base64.b64encode(mac).decode('utf-8')

    # Encrypted message
    output = json.dumps({'nonce':nonce, 'ciphertext':ct, 'salt':saltsafe, 'tag':tag})
...
def file_decrypt(secret, output):
    ...
    mac = hmac.new(kmac, ct, hashlib.sha256).digest()
